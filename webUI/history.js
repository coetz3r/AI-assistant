...
function matchesFilters(entry, search, backend, outcome) {
  if (backend && entry.backend !== backend) return false;
  if (outcome && entry.outcome !== outcome) return false;
  if (search) {
    const haystack = ((entry.user_text || '') + ' ' + (entry.reply_text || '')).toLowerCase();
...
function renderTable() {
  ...
  const filtered = allEntries.filter(e => matchesFilters(e, search, backend, outcome));
  const body = document.getElementById('historyBody');

  if (!filtered.length) {
    body.innerHTML = '<tr><td colspan="11" class="empty-note">No turns match.</td></tr>';
    return;
  }

  body.innerHTML = filtered.map(t => (
    '<tr class="history-row" title="' + escapeHtml(t.user_text || '') + ' → ' + escapeHtml(t.reply_text || '') + '">' +
...
async function loadInitial() {
  ...
  try {
    ...
    document.getElementById('historyBody').innerHTML =
      '<tr><td colspan="11" class="empty-note">Couldn\'t load history — is the server running?</td></tr>';
    hasMore = false;
  }
  updateMeta();
  renderTable();
...
document.getElementById('historySearch').addEventListener('input', renderTable);
